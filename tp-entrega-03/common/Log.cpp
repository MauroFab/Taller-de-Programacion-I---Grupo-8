...
void Log::cargarConfiguracion(){
	...
	string defaultValues[] = {TP_ERROR, TP_WARN, TP_INFO, TP_DEBUG};
...
void Log::debug(string mensaje){
	escribirMensaje(TP_DEBUG, mensaje);
}

void Log::info(string mensaje){
	escribirMensaje(TP_INFO, mensaje);
}

void Log::warn(string mensaje){
	escribirMensaje(TP_WARN, mensaje);
}

void Log::error(string mensaje){
	escribirMensaje(TP_ERROR, mensaje);
}
